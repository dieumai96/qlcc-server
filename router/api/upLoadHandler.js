...
function uploadS3Small(fp, path, key, result, reply) {
    sharp(fp)
        .resize(280, 160)
        .max()
...
uploadHandler.uploadFiles = (req, reply) => {
...
uploadHandler.uploadAvatarFiles = (req, reply) => {
    let form = new multiparty.Form();
    form.parse(req.payload, (err, fields, files) => {
        let newFileMaps = [];
        if (typeof (files) === 'undefined') {
        ...
        fs.readFile(fp, (err, data) => {
            ...
            fs.writeFile(newpath, data, (err) => {
                if (err) log.info(err);
                    ...
                    uploadS3Small(fp, newpath, fn[0], result, reply);
                }
            })
        });
    });
